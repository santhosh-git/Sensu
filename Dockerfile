...
#&& sudo wget http://www.rabbitmq.com/releases/rabbitmq-server/v3.6.0/rabbitmq-server_3.6.0-1_all.deb && sudo dpkg -i rabbitmq-server_3.6.0-1_all.deb

#install postfix
#RUN debconf-set-selections <<< "postfix postfix/mailname string powerupcloud.com" && debconf-set-selections <<< "postfix postfix/main_mailer_type string 'Internet Site'" && apt-get install mailutils postfix -y
